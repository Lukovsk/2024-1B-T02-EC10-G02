...
class OrderService():
    def __init__(self, id=None, medicationId=None, status=None, senderId=None, receiverId=None, feedbackId=None, canceledAt=None, canceledBy=None, canceled_reason=None):
        self.id = id
    ...
    async def get_all(self) -> list[Prisma.order]:
    ...
    async def get_closed_orders(self) -> list[Prisma.order]:
        async with self.database_connection():
            try:
                orders = await self.db.order.find_many(
                    where={
                ...
                ) if self.senderId else await self.db.order.find_many(
                    where={
                        "deleted": False,
                        "status": "CLOSED", # TODO: change closed status to some enum from prisma
    ...
    async def get_open_orders(self) -> list[Prisma.order]:
        async with self.database_connection():
            try:
                orders = await self.db.order.find_many(
                    where={
    ...
    ## Eu quero um pedido
    async def get_by_id(self) -> Prisma.order:
        async with self.database_connection():
            try:
    ...
    async def create(self) -> Prisma.order:
    ...
    async def update(self, new_data) -> Prisma.order:
        """update an order
